...
can create this "exactly one option" constraint without sacrificing the strictly typed nature of our GraphQL Schema. In code-first schemas, this is expressed with `isOneOf: true`. In SDL, it is expressed with `@oneOf`.
  ...
  query: new GraphQLObjectType({
    ...
    fields: {
      product: {
        ...
        resolve: (_source, { by }) => findProduct(by),
      },
    },
  }),
});
  ...
  `,
  {
    supplementalConfig: {
      objectTypes: {
        Query: {
          fields: {
            product: (_source, { by }) => findProduct(by),
          },
        },
      },
    },
