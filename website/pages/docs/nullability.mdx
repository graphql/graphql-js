...
## The role of non-null types

SDL represents non-nullability with `!`. In code-first schemas, GraphQL.js represents
the same concept with the `GraphQLNonNull` wrapper type.
