...
function that can fetch multiple users in a single call. The SDL example attaches the
`Post.author` resolver through `supplementalConfig` so every `Post` value can load its
author through `context.userLoader`.
