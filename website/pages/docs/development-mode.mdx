...
2. implicitly, by setting the 'development' condition, which is possible only on
   platforms that support `package.json` conditional exports and custom conditions.
...
We encourage enabling development mode in a development environment, either
explicitly or implicitly. This allows for an additional check to ensure that only a
single GraphQL.JS module is used. Additional development-time checks may also be added in
the future.
