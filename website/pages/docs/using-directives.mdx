...
`greeting` field in this example is skipped entirely and your resolver won't run. The same
runtime behavior works with code-first schema construction or SDL:
...
const source = `  query ($shouldInclude: Boolean!) {
    greeting @include(if: $shouldInclude)
  }`;
...
const result = await graphql({
  schema,
  source,
  variableValues: { shouldInclude: true },
});
...
To use a custom directive, you first define it in your schema. A directive in GraphQL.js is
metadata: it doesn't perform any behavior on its own. Define it with `GraphQLDirective` in
code-first schemas, or with a directive definition in SDL.
...
const schema = new GraphQLSchema({
  ...
  directives: [...specifiedDirectives, UppercaseDirective],
});
```
...
const schema = buildSchema(/* GraphQL */ `
  "Converts the result of a field to uppercase."
  directive @uppercase(
    "Whether to apply the transformation."
...
Once declared in the schema, tools like validation and introspection will recognize it.
    ...
    fields: {
      greeting: {
        ...
        resolve: (_source, _args, _context, info) => {
          ...
          return directive?.enabled ? result.toUpperCase() : result;
        },
      },
    },
  }),
...
```js
import { buildSchema, getDirectiveValues, graphql } from 'graphql';
  ...
  `,
  {
    supplementalConfig: {
      objectTypes: {
        Query: {
          fields: {
            greeting: (_source, _args, _context, info) => {
              const directive = getDirectiveValues(
    ...
    fields: {
      greeting: {
        ...
        resolve: (_source, args) => {
          ...
          return args.format === 'UPPERCASE' ? result.toUpperCase() : result;
        },
      },
    },
  }),
  ...
  `,
  {
    supplementalConfig: {
      objectTypes: {
        Query: {
          fields: {
            greeting: (_source, args) => {
              const result = 'Hello, world';

              return args.format === 'UPPERCASE'
                ? result.toUpperCase()
                : result;
            },
          },
