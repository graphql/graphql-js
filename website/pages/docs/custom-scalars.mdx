...
If you previously relied on `parseLiteral(ast, variables)` for custom variable
replacement inside complex scalar literals, use `replaceVariables()` before
calling `coerceInputLiteral()`.
