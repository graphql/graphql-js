...
- Use snapshot tests to catch unintended schema changes  
  ...
  – Example:
    ...
    ```
    - Consider sorting the schema (`graphql.lexicographicSortSchema(schema)`) to ensure
      a stable order of your schemas types, fields and arguments in the snapshots.
