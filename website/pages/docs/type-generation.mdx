...
Another workflow is to define the schema in SDL and supply JavaScript-only behavior through
`supplementalConfig`. That gives type-generation tools SDL directly while still letting you
attach resolvers, custom scalar coercion, abstract type resolution, and extension metadata
in code.
...
You just need to convert your schema into SDL. If the schema is already constructed from
SDL, that conversion step is unnecessary.
...
Once you have SDL, you can use it as the input for type generation.
