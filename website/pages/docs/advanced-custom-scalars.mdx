...
In v17, `coerceInputLiteral()` receives a constant literal. If a custom scalar
tool needs to replace variables inside a value before calling that method, use
the exported `replaceVariables()` helper first.
