...
spawn('git', ['add', 'package.json', 'package-lock.json', 'src/version.js']);
...
function throwUsage(message) {
  throw new Error(
    `${message}\n` +
      ...
      '  npm run release:prepare -- 15.x.x prerelease --preid alpha\n' +
      '  npm run release:prepare -- --fromRev <fromRev> 15.x.x prerelease --preid alpha',
  );
}
