...
function ValuesOfCorrectTypeRule(context) {
    ...
    return {
        ...
        ListValue(node) {
            ...
            const type = (0, definition_js_1.getNullableType)(context.getParentInputType());
        ...
        ObjectValue(node) {
            ...
            for (const fieldDef of Object.values(type.getFields())) {
                ...
                if (!fieldNode && (0, definition_js_1.isRequiredInputField)(fieldDef)) {
                    ...
                    context.reportError(new GraphQLError_js_1.GraphQLError(`Field "${type}.${fieldDef.name}" of required type "${typeStr}" was not provided.`, { nodes: node }));
                }
            }
            if (type.isOneOf) {
        ...
        NullValue(node) {
            ...
            if ((0, definition_js_1.isNonNullType)(type)) {
                context.reportError(new GraphQLError_js_1.GraphQLError(`Expected value of type "${(0, inspect_js_1.inspect)(type)}", found ${(0, printer_js_1.print)(node)}.`, { nodes: node }));
            }
        },
...
function isValidValueNode(context, node) {
    ...
    try {
        ...
        if (error instanceof GraphQLError_js_1.GraphQLError) {
            ...
            context.reportError(new GraphQLError_js_1.GraphQLError(`Expected value of type "${typeStr}", found ${(0, printer_js_1.print)(node)}; ` +
...
function validateOneOfInputObject(context, node, type, fieldNodeMap, variableDefinitions) {
    ...
    if (isNotExactlyOneField) {
        context.reportError(new GraphQLError_js_1.GraphQLError(`OneOf Input Object "${type}" must specify exactly one key.`, { nodes: [node] }));
        return;
    }
    ...
    if (isNullLiteral) {
        context.reportError(new GraphQLError_js_1.GraphQLError(`Field "${type}.${keys[0]}" must be non-null.`, {
            nodes: [node],
        }));
        return;
    }
    if (isVariable) {
        ...
        const isNullableVariable = definition.type.kind !== kinds_js_1.Kind.NON_NULL_TYPE;
        if (isNullableVariable) {
            context.reportError(new GraphQLError_js_1.GraphQLError(`Variable "$${variableName}" must be non-nullable to be used for OneOf Input Object "${type}".`, { nodes: [node] }));
        }
    }
}
