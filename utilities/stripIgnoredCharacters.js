...
function stripIgnoredCharacters(source) {
    ...
    while (lexer.advance().kind !== tokenKind_js_1.TokenKind.EOF) {
         ...
         */
        const isNonPunctuator = !(0, lexer_js_1.isPunctuatorTokenKind)(currentToken.kind);
        if (wasLastAddedTokenNonPunctuator) {
            if (isNonPunctuator || currentToken.kind === tokenKind_js_1.TokenKind.SPREAD) {
                strippedBody += ' ';
            }
        }
        const tokenBody = body.slice(currentToken.start, currentToken.end);
        if (tokenKind === tokenKind_js_1.TokenKind.BLOCK_STRING) {
            strippedBody += (0, blockString_js_1.printBlockString)(currentToken.value, { minimize: true });
        }
        else {
