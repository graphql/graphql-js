...
function collectConflictsBetweenFieldsAndFragment(
  ...
  const fragmentName = fragmentSpread.node.name.value;
  ...
  for (const referencedFragmentSpread of Object.values(
...
function getFieldsAndFragmentSpreads(
  ...
  const fragmentSpreads: ObjMap<FragmentSpread> = Object.create(null);
  ...
  const result: FieldsAndFragmentSpreads = [
    nodeAndDefs,
    Array.from(Object.values(fragmentSpreads)),
  ];
...
function _collectFieldsAndFragmentNames(
  ...
  fragmentSpreads: ObjMap<FragmentSpread>,
  varMap: Map<string, ValueNode> | undefined,
): void {
  for (const selection of selectionSet.selections) {
    switch (selection.kind) {
      ...
      case Kind.FRAGMENT_SPREAD: {
        ...
        fragmentSpreads[fragmentSpread.key] = fragmentSpread;
        break;
      }
