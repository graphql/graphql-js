  ...
  it('rejects a Scalar type defining serialize with an incorrect type', () => {
    expect(
      () =>
        new GraphQLScalarType({
          name: 'SomeScalar',
          // @ts-expect-error - invalid serialize function
          serialize: {},
        }),
    ).to.throw(
...
describe('Type System: Objects', () => {
  ...
  it('rejects an Object type with a field function that returns incorrect type', () => {
    const objType = new GraphQLObjectType({
      name: 'SomeObject',
      // @ts-expect-error - invalid type returned
      fields() {
  ...
  it('rejects an Object type with an isDeprecated instead of deprecationReason on field', () => {
    const OldObject = new GraphQLObjectType({
      name: 'OldObject',
      fields: {
        // @ts-expect-error - invalid definition, should use deprecationReason
  ...
  it('rejects an empty Object field resolver', () => {
    const objType = new GraphQLObjectType({
      name: 'SomeObject',
      fields: {
        // @ts-expect-error - invalid resolver
    ...
    expect(
      () =>
        new GraphQLObjectType({
          name: 'AnotherObject',
          fields: {},
          // @ts-expect-error - invalid isTypeOf function
          isTypeOf: {},
        }),
    ).to.throw(
    ...
    expect(
      () =>
        new GraphQLInterfaceType({
          name: 'AnotherInterface',
          fields: {},
          // @ts-expect-error - invalid resolveType function
          resolveType: {},
        }),
    ).to.throw(
...
describe('Type System: Unions', () => {
  ...
  it('rejects a Union type with incorrectly typed types', () => {
    const unionType = new GraphQLUnionType({
      name: 'SomeUnion',
      // @ts-expect-error - invalid types field
      types: { ObjectType },
    });
    ...
    expect(
      () =>
        new GraphQLEnumType({
          name: 'SomeEnum',
          values: {
            // @ts-expect-error - invalid usage of deprecation
  ...
  describe('Input Object fields must not have resolvers', () => {
    it('rejects an Input Object type with resolvers', () => {
      const inputObjType = new GraphQLInputObjectType({
        name: 'SomeInputObject',
        fields: {
          // @ts-expect-error - reject resolver
...
describe('Type System: List', () => {
  ...
  it('rejects a non-type as item type of list', () => {
    // @ts-expect-error - rejects object
    expectList({}).to.throw('Expected {} to be a GraphQL type.');
    // @ts-expect-error - rejects String
