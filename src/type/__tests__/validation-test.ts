...
function withModifiers<T extends GraphQLType>(
  type: T,
): Array<T | GraphQLList<any> | GraphQLNonNull<any>> {
  return [
    type,
    GraphQLList(type),
    GraphQLNonNull(type),
    GraphQLNonNull(new GraphQLList(type)),
  ];
}
...
function schemaWithFieldType(type: GraphQLNamedType | GraphQLOutputType) {
  return new GraphQLSchema({
    query: new GraphQLObjectType({
      name: 'Query',
      fields: { f: { type: type as GraphQLOutputType } },
    }),
...
describe('Type System: Enum types must be well defined', () => {
  ...
  it('rejects an Enum type with incorrectly named values', () => {
    function schemaWithEnum(values: GraphQLEnumValueConfigMap) {
...
describe('Type System: Objects can only implement unique interfaces', () => {
  it('rejects an Object implementing a non-type values', () => {
    const schema = new GraphQLSchema({
      query: new GraphQLObjectType({
        name: 'BadObject',
        // @ts-expect-error - invalid value for an interface
...
describe('Type System: Interface fields must have output types', () => {
  ...
  it('rejects an empty Interface field type', () => {
    // @ts-expect-error - empty interface array
