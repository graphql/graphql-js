...
const fieldNode = (operationNode as OperationDefinitionNode).selectionSet
  .selections[0];
invariant(fieldNode);
...
describe('printError', () => {
  ...
  it('prints an error with nodes from different sources', () => {
    ...
    const fieldA = (opA as ObjectTypeDefinitionNode).fields![0];
    ...
    const fieldB = (opB as ObjectTypeDefinitionNode).fields![0];
