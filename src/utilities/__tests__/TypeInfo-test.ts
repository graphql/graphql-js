...
describe('visitWithTypeInfo', () => {
  it('supports different operation types', () => {
    ...
    const rootTypes: Record<string, any> = {};
    visit(
      ast,
  ...
  it('provide exact same arguments to wrapped visitor', () => {
    ...
    const visitorArgs: Array<['enter' | 'leave', ...Array<any>]> = [];
    visit(ast, {
      enter(...args) {
    ...
    const wrappedVisitorArgs: Array<['enter' | 'leave', ...Array<any>]> = [];
    const typeInfo = new TypeInfo(testSchema);
    visit(
      ast,
      visitWithTypeInfo(typeInfo, {
      ...
      visitWithTypeInfo(typeInfo, {
        enter(node) {
          ...
          if (
            node.kind === 'Field' &&
            !node.selectionSet &&
            isCompositeType(getNamedType(type as GraphQLType))
          ) {
            return {
              ...node,
              selectionSet: {
  ...
  it('supports traversals of input values', () => {
    ...
    const typeInfo = new TypeInfo(
      testSchema,
      undefined,
      complexInputType as GraphQLType,
    );

    const visited: Array<[string, string, string | null, string]> = [];
    visit(
      ast,
      visitWithTypeInfo(typeInfo, {
        enter(node) {
  ...
  it('supports traversals of selection sets', () => {
    ...
    const visited: Array<[string, string, string | null, string, string]> = [];
    visit(
      (operationNode as OperationDefinitionNode).selectionSet,
      visitWithTypeInfo(typeInfo, {
        enter(node) {
          const parentType = typeInfo.getParentType();
          const type = typeInfo.getType();
