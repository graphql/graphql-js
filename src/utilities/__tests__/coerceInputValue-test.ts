...
type ErrorsArray = Array<{
  path: ReadonlyArray<string | number>;
  value: any;
  error: string;
}>;

function expectValue(result: { errors?: ErrorsArray; value?: any }) {
  expect(result.errors).to.deep.equal([]);
  return expect(result.value);
}

function expectErrors(result: { errors?: ErrorsArray }) {
  return expect(result.errors);
}

describe('coerceInputValue', () => {
  function coerceValue<V>(inputValue: V, type: GraphQLInputType) {
  ...
  describe('for GraphQLInputObject with default value', () => {
    const TestInputObject = <T>(defaultValue: T) =>
