...
import type { DefinitionNode, ASTNode } from '../../language/ast';
import {
  buildASTSchema,
  buildSchema,
  BuildSchemaOptions,
 ...
 */
function cycleSDL(sdl: string, options?: BuildSchemaOptions) {
  const ast = parse(sdl);
...
function printASTNode(obj?: null | { astNode?: null | ASTNode }) {
  invariant(obj?.astNode != null);
  // @ts-expect-error - might be null, but we know it's there
  return print(obj.astNode);
}

function printAllASTNodes(obj: GraphQLNamedType) {
  invariant(obj.astNode != null && obj.extensionASTNodes != null);
  return print({
    kind: Kind.DOCUMENT,
    definitions: [
      obj.astNode as DefinitionNode,
      ...(obj.extensionASTNodes as Array<DefinitionNode>),
    ],
  });
}
