...
export function validateInputValue(
  ...
  return validateInputValueImpl(inputValue, type, onError, undefined);
}
...
function validateInputValueImpl(
  ...
  if (isListType(type)) {
    if (!isIterableObject(inputValue)) {
      ...
      validateInputValueImpl(inputValue, type.ofType, onError, path);
    } else {
      let index = 0;
...
export function validateInputLiteral(
  ...
  return validateInputLiteralImpl(context, valueNode, type, undefined);
}
...
function validateInputLiteralImpl(
  ...
  if (isNonNullType(type)) {
    ...
    return validateInputLiteralImpl(context, valueNode, type.ofType, path);
  }
  ...
  if (isListType(type)) {
    if (valueNode.kind !== Kind.LIST) {
      // Lists accept a non-list value as a list of one.
      validateInputLiteralImpl(context, valueNode, type.ofType, path);
    } else {
      let index = 0;
    ...
    let caughtError;
    try {
      result = type.parseConstLiteral(constValueNode);
    } catch (error) {
      if (error instanceof GraphQLError) {
