...
function findInputObjectTypeChanges(
  ...
  for (const newField of fieldsDiff.added) {
    if (isRequiredInputField(newField)) {
      schemaChanges.push({
        ...
        description: `A required field ${oldType.name}.${newField.name} was added.`,
      });
    } else {
      schemaChanges.push({
        type: DangerousChangeType.OPTIONAL_INPUT_FIELD_ADDED,
        description: `An optional field ${oldType.name}.${newField.name} was added.`,
      });
    }
  }
  ...
  for (const oldField of fieldsDiff.removed) {
    schemaChanges.push({
      ...
      description: `Field ${oldType.name}.${oldField.name} was removed.`,
    });
  }
  ...
  for (const [oldField, newField] of fieldsDiff.persisted) {
    ...
    if (!isSafe) {
      schemaChanges.push({
        ...
        description:
          `Field ${oldType.name}.${oldField.name} changed type from ` +
...
function findEnumTypeChanges(
  ...
  for (const newValue of valuesDiff.added) {
    schemaChanges.push({
      ...
      description: `Enum value ${oldType.name}.${newValue.name} was added.`,
    });
  }
  ...
  for (const oldValue of valuesDiff.removed) {
    schemaChanges.push({
      ...
      description: `Enum value ${oldType.name}.${oldValue.name} was removed.`,
    });
  }
...
function findArgChanges(
  ...
  for (const oldArg of argsDiff.removed) {
    schemaChanges.push({
      ...
      description: `Argument ${oldType.name}.${oldField.name}(${oldArg.name}:) was removed.`,
    });
  }
  ...
  for (const [oldArg, newArg] of argsDiff.persisted) {
    ...
    if (!isSafe) {
      schemaChanges.push({
        ...
        description:
          `Argument ${oldType.name}.${oldField.name}(${oldArg.name}:) has changed type from ` +
      ...
      if (newArg.defaultValue === undefined) {
        schemaChanges.push({
          ...
          description: `${oldType.name}.${oldField.name}(${oldArg.name}:) defaultValue was removed.`,
        });
      } else {
        ...
        if (oldValueStr !== newValueStr) {
          schemaChanges.push({
            type: DangerousChangeType.ARG_DEFAULT_VALUE_CHANGE,
            description: `${oldType.name}.${oldField.name}(${oldArg.name}:) has changed defaultValue from ${oldValueStr} to ${newValueStr}.`,
          });
        }
      }
    }
  ...
  for (const newArg of argsDiff.added) {
    if (isRequiredArgument(newArg)) {
      schemaChanges.push({
        ...
        description: `A required argument ${oldType.name}.${oldField.name}(${newArg.name}:) was added.`,
      });
    } else {
      schemaChanges.push({
        type: DangerousChangeType.OPTIONAL_ARG_ADDED,
        description: `An optional argument ${oldType.name}.${oldField.name}(${newArg.name}:) was added.`,
      });
    }
  }
