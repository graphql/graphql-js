...
export function coerceValue(
  ...
  if (isInputUnionType(type)) {
    ...
    const inputName =
      typeof value.__inputname !== 'string' ? '' : value.__inputname;
    const validTypeMap = type.getTypeMap();
    if (!inputName || !validTypeMap[inputName]) {
      return ofErrors([
        coercionError(
          `Expected ${type.name}.__inputname to be one of ${Object.keys(
            validTypeMap,
          ).join(', ')}. Saw "${inputName}"`,
          blameNode,
          path,
...
function coerceObject(type, value, path, blameNode, coercedValue = {}) {
  let errors;
