...
const InboxType = new GraphQLObjectType({
  ...
  fields: {
    ...
    unread: {
      type: GraphQLInt,
      resolve: (inbox) =>
        inbox.emails.filter((email: Email) => email.unread).length,
    },
...
function emailSchemaWithResolvers(
  subscribeFn?: GraphQLFieldResolver<any, any>,
...
interface TestData {
  ...
  importantEmail: () => AsyncIterator<any>;
}
...
async function createSubscription(
  ...
  sendImportantEmail: (e: Email) => boolean;
  subscription: AsyncIterableIterator<any>;
}> {
  const data: TestData = {
    inbox: {
      emails: [
  ...
  function sendImportantEmail(newEmail: Email) {
...
describe('Subscription Initialization Phase', () => {
  ...
  it('should only resolve the first field of invalid multi-field', async () => {
    ...
    // FIXME: https://github.com/graphql/graphql-js/issues/2610
  ...
  it('throws an error if schema is missing', async () => {
    ...
    await expectPromiseToThrow(
      // @ts-expect-error - schema is null
      () => subscribe(null, document),
      'Expected null to be a GraphQL schema.',
    );

    await expectPromiseToThrow(
      // @ts-expect-error - no schema
      () => subscribe({ document }),
...
describe('Subscription Publish Phase', () => {
  ...
  it('produces a payload per subscription event', async () => {
    ...
    // @ts-expect-error - return might be empty, but we are aware of that
  ...
  it('should not trigger when subscription is thrown', async () => {
    ...
    let caughtError;
    try {
      // @ts-expect-error - throw might be empty in the source declaration
