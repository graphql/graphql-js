...
> {
  ...
  events: AsyncGenerator<ReadonlyArray<WorkQueueEvent<T, I, G, S>>>;
}
...
/** @internal **/
export class Task<T, I, G extends Group<G>, S extends Stream<T, I, G, S>> {
  readonly groups: ReadonlyArray<G>;
  private _fn: () => PromiseOrValue<TaskResult<T, I, G, S>>;
  private _maybePromise?: MaybePromise<TaskResult<T, I, G, S>>;
  constructor(
    fn: () => PromiseOrValue<TaskResult<T, I, G, S>>,
    groups: ReadonlyArray<G>,
  ) {
    this._fn = fn;
    this.groups = groups;
  }
  start(): MaybePromise<TaskResult<T, I, G, S>> {
  ...
  result(): PromiseOrValue<TaskResult<T, I, G, S>> {
