...
import type { GraphQLCompiledSchema } from './compiledSchema';
...
export class GraphQLExecution {
  ...
  constructor(config: GraphQLExecutionConfig) {
    this.compiledSchema = config.compiledSchema;
    this.exeContext = config.exeContext;
    this.rootValue = config.rootValue;
    this.contextValue = config.contextValue;
    this.variableValues = config.variableValues;
    this.errors = [];
  }
  ...
  executeQueryOrMutation(
    ...
    try {
      ...
      if (isPromise(result)) {
        return result.then(
          (data) => this.buildResponse(data, this.errors),
          (error) => {
            this.errors.push(error);
            return this.buildResponse(null, this.errors);
          },
        );
      }
      return this.buildResponse(result, this.errors);
    } catch (error) {
      this.errors.push(error);
      return this.buildResponse(null, this.errors);
    }
  ...
  executeQueryOrMutationRootFields(
    ...
    const { operation, fragments } = exeContext;
    const rootType = this.compiledSchema.schema.getRootType(
      operation.operation,
    );
    if (rootType == null) {
  ...
  executeField(
    ...
    const fieldDef = this.compiledSchema.schema.getField(parentType, fieldName);
    if (!fieldDef) {
      return;
    }

    const returnType = fieldDef.type;
    const resolveFn = fieldDef.resolve ?? this.compiledSchema.fieldResolver;
    ...
    while (true) {
      ...
      try {
        ...
        try {
          ...
          this.handleFieldError(error, itemType);
        }
      } catch (rawError) {
        completedResults.push(null);
        const error = locatedError(
  ...
  completeAbstractValue(
    ...
    const resolveTypeFn =
      returnType.resolveType ?? this.compiledSchema.typeResolver;
    const contextValue = this.contextValue;
  ...
  ensureValidRuntimeType(
    ...
    const runtimeType = this.compiledSchema.schema.getType(runtimeTypeName);
    if (runtimeType == null) {
      throw new GraphQLError(
    ...
    if (!this.compiledSchema.schema.isSubType(returnType, runtimeType)) {
      throw new GraphQLError(
  ...
  completeObjectValue(
    ...
    if (returnType.isTypeOf) {
      const isTypeOf = returnType.isTypeOf(result, this.contextValue, info);
  ...
  > {
    ...
    return mapAsyncIterator(resultOrStream, (payload: unknown) => {
      const eventExecution = new GraphQLExecution({
      ...
      return eventExecution.executeQuery(exeContext);
    });
  }
  ...
  executeSubscriptionRootField(
    ...
    const rootType = this.compiledSchema.schema.getSubscriptionType();
    if (rootType == null) {
      throw new GraphQLError(
    ...
    const fieldName = fieldNodes[0].name.value;
    const fieldDef = this.compiledSchema.schema.getField(rootType, fieldName);
    ...
    try {
      ...
      const resolveFn =
        fieldDef.subscribe ?? this.compiledSchema.subscribeFieldResolver;
      const result = resolveFn(this.rootValue, args, contextValue, info);
