...
import { createWorkQueue, Task } from '../WorkQueue.js';
...
function makeTask(
  ...
  if (typeof valueOrFactory === 'function') {
    return new Task(valueOrFactory, groups);
  }
  return new Task(() => ({ value: valueOrFactory, work }), groups);
}
...
describe('Task', () => {
  it('can return a result', () => {
    const task = makeTask([], () => ({ value: 123 }));

    expect(task.result()).to.deep.equal({ value: 123 });
  });
  ...
  it('only runs once when started multiple times', async () => {
    ...
    await Promise.all([task.start(), task.start(), task.start()]);
    const results = [task.result(), task.result(), task.result()];
  ...
  it('stores sync error in result()', () => {
    ...
    expect(() => task.start()).to.not.throw();
    expect(() => task.result()).to.throw('failure');
    expect(() => task.result()).to.throw('failure');
    expect(runCount).to.equal(1);
  });
  ...
  it('stores async error in result()', async () => {
    ...
    expect(() => task.start()).to.not.throw();
    await expectPromise(task.result()).toRejectWith('failure');
    expect(() => task.result()).to.throw('failure');
    expect(runCount).to.equal(1);
  });
});
...
describe('WorkQueue', () => {
  ...
  it('handles tasks that are started manually before they complete', async () => {
    const group: TestGroup = { parent: undefined };
    const primedTask = makeTask([group], async () => {
      await resolveOnNextTick();
      return { value: 'primed' };
    });
  ...
  it('handles tasks that are already settled before being queued', async () => {
    const group: TestGroup = { parent: undefined };
    const eagerTask = makeTask([group], 'eager');
