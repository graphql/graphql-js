...
import { ASTNode, SelectionSetNode } from '../ast';

function checkVisitorFnArgs(
  ast: ASTNode,
  args: IArguments,
  isEdited?: boolean,
) {
  ...
  if (!isEdited) {
    ...
    for (let i = 0; i < ancestors.length; ++i) {
      ...
      currentNode = (currentNode as any)[path[i]];
...
describe('Visitor', () => {
  it('validates path argument', () => {
    const visited: Array<[string, Array<string | number>]> = [];
  ...
  it('validates ancestors argument', () => {
    const ast = parse('{ a }', { noLocation: true });
    const visitedNodes: Array<
      ASTNode | ReadonlyArray<ASTNode> | undefined
    > = [];
  ...
  it('allows skipping a sub-tree', () => {
    const visited: Array<[string, string, string | undefined]> = [];
  ...
  it('visits kitchen sink', () => {
    ...
    const argsStack: Array<Array<IArguments>> = [];
  ...
  describe('Support for custom AST nodes', () => {
    ...
    // @ts-expect-error - trying to access union of objects, but we know it's there
  ...
  describe('visitInParallel', () => {
    ...
    it('allows skipping different sub-trees', () => {
      const visited: Array<[string, string, string, string | undefined]> = [];
